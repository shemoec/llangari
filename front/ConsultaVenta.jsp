    ...
    <body>
        ...
        <div class="d-flex">
            ...
            <div class="panel-body table-responsive" class="box-body">
                <br>                
                &nbsp;&nbsp;&nbsp;&nbsp;<strong><font size= "+2">Consulta de Egresos</font></strong>
                <br><br>
                <form action="Controlador?menu=ConsultaVenta" method="post">
                    <div class="col-lg-6">
                        <label>Fecha Inicio:</label>
                        <input type="date" name="txtFechaInicioVenta" pattern="yyyy-MM-dd">
                        &nbsp;&nbsp;&nbsp;&nbsp;<label>Fecha Fin:</label>
                        <input type="date" name="txtFechaFinVenta" pattern="yyyy-MM-dd">
                        &nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="accion" value="Listar" class="btn btn-info">
                    </div>
                </form>
                <br><br>
                <div class="col-sm-12">
                    <table id="tblventas" class="table table-striped table-bordered table-condensed table-hover" width="100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Cliente</th>
                        ...
                        <tbody>
                            <c:forEach var="vent" items="${ventas}">
                                <tr>
                                    <td>${vent.getIdVenta()}</td>
                                    <td>${vent.getCliente()}</td>
                                    <td>${vent.getUsuario()}</td>
                                    <td>${vent.getNumSerie()}</td>
                                    <td>${vent.getFecha()}</td>
                                    <td>${vent.getTotal()}</td>
                                    <c:if test="${vent.estado == 1}">
                                        <td><span style='font-size:14px;' class='label label-success'>Pagada</span></td>
                                    </c:if>
                                    <c:if test="${vent.estado == 0}">
                                        <td><span style='font-size:13px;' class='label label-danger'>Cancelada</span></td>
                                    </c:if>

                                    <c:if test="${vent.estado == 1}">
                                        <td>
                                            <a class="btn btn-danger" href="Controlador?menu=ConsultaVenta&accion=Anular&id=${vent.getIdVenta()}"><i class="fa fa-ban"></i></a>
                                        </td>
                                    </c:if>
        ...
        .appendTo( '#tblventas_wrapper .col-md-6:eq(0)' );
        } );
