<%@page import="modelo.dao.RolDAO"%>
<%@page import="modelo.entidades.Rol"%>
    ...
    <body>
        ...
        <div class="d-flex">
            ...
            <div class="panel-body table-responsive" class="box-body">
                <br>                
                &nbsp;&nbsp;&nbsp;&nbsp;<strong><font size= "+2">Usuarios</font></strong>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalNuevo"> Nuevo</button>
                <br><br>
                <div class="col-sm-12">
                    <table id="tblusuario" class="table table-striped table-bordered table-condensed table-hover" width="100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Cédula</th>
                        ...
                        <tbody>
                            <c:forEach var="us" items="${usuarios}">
                                <tr>
                                    ...
                                    <td>${us.getCorreo()}</td>
                                    <c:if test="${us.estado == 1}">
                                        <td><span style='font-size:14px;' class='label label-success'>Activo</span></td>
                                    </c:if>
                                    <c:if test="${us.estado == 0}">
                                        <td><span style='font-size:13px;' class='label label-danger'>Inactivo</span></td>
                                    </c:if>

                                    <c:if test="${us.estado == 1}">
                                        <td>
                                            <button type="button"  onclick="return modaleditar(${us.getIdUsuario()})" class="btn btn-warning" data-toggle="modal" data-target="#ModalEditar"><i class="fa fa-edit"></i></button>
                                            <a class="btn btn-danger" href="Controlador?menu=Usuario&accion=Desactivar&id=${us.getIdUsuario()}"><i class="fa fa-ban"></i></a>
                                        </td>
                                    </c:if>
                                    <c:if test="${us.estado == 0}">                                        
                                        <td>
                                            <button type="button"  onclick="return modaleditar(${us.getIdUsuario()})" class="btn btn-warning" data-toggle="modal" data-target="#ModalEditar"><i class="fa fa-edit"></i></button>
                                            <a class="btn btn-success" href="Controlador?menu=Usuario&accion=Activar&id=${us.getIdUsuario()}"><i class="fa fa-check"></i></a>
                                        </td>
                                    </c:if>                                
                                </tr>
                            </c:forEach>
        ...
        <script>
                                                ...
                                                $('.modal-body').load('editarUsuario.jsp?id=' + id, function () {});
                                                }
        ...
        .appendTo( '#tblusuario_wrapper .col-md-6:eq(0)' );
        } );
