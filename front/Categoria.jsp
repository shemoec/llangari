    ...
    <body>
        ...
        <div class="d-flex">
            ...
            <div class="panel-body table-responsive" class="box-body">
                <br>                
                &nbsp;&nbsp;&nbsp;&nbsp;<strong><font size= "+2">Categorias</font></strong>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#ModalNuevo"> Nuevo</button>
                <br><br>
                <div class="col-sm-12">
                    <table id="tblcategoria" class="table table-striped table-bordered table-condensed table-hover" width="100%">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Nombre</th>
                        ...
                        <tbody>
                            <c:forEach var="cat" items="${categorias}">
                                <tr>
                                    ...
                                    <c:if test="${cat.estado == 1}">
                                        <td><span style='font-size:14px;' class='label label-success'>Activo</span></td>
                                    </c:if>
                                    <c:if test="${cat.estado == 0}">
                                        <td><span style='font-size:13px;' class='label label-danger'>Inactivo</span></td>
                                    </c:if>

                                    <c:if test="${cat.estado == 1}">
                                        <td>
                                            <button type="button"  onclick="return modaleditar(${cat.getIdCategoria()})" class="btn btn-warning" data-toggle="modal" data-target="#ModalEditar"><i class="fa fa-edit"></i></button>
                                            <a class="btn btn-danger" href="Controlador?menu=Categoria&accion=Desactivar&id=${cat.getIdCategoria()}"><i class="fa fa-ban"></i></a>
                                        </td>
                                    </c:if>
                                    <c:if test="${cat.estado == 0}">                                        
                                        <td>
                                            <button type="button"  onclick="return modaleditar(${cat.getIdCategoria()})" class="btn btn-warning" data-toggle="modal" data-target="#ModalEditar"><i class="fa fa-edit"></i></button>
                                            <a class="btn btn-success" href="Controlador?menu=Categoria&accion=Activar&id=${cat.getIdCategoria()}"><i class="fa fa-check"></i></a>
                                        </td>
                                    </c:if>                                
                                </tr>
                            </c:forEach>
        ...
        .appendTo( '#tblcategoria_wrapper .col-md-6:eq(0)' );
        } );
